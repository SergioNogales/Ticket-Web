    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") 
    {
        ...
        else
        {
            foreach ($usuarios as $usuario) 
            {
                ...
                {
                    $tempUser2 = new usuario( $usuario['email'], $usuario['password'], $usuario['rol'], $usuario['telefono'], $usuario['direccion'], $usuario['localidad'], $usuario['nombre'], $usuario['codigo_postal'], $usuario['tarjeta'], $usuario['mes_caducidad'], $usuario['year_caducidad'], $usuario['ccv'], $usuario['saldo'], $usuario['solicitud']);
        ...
        <section class="main eventos admin">
            ...
            <?php
                ...
                foreach ($usuarios as $usuario) {
                    ...
                    $solicitud = htmlspecialchars($usuario['solicitud']);

                    $estado = "";
                    if (strpos($solicitud, "_pendiente") !== false) 
                    {
                        $nuevoRol = str_replace("_pendiente", "", $solicitud);
                        $estado = "pendiente";
                    } 
                    if (strpos($solicitud, "_aprobada") !== false) 
                    {
                        $nuevoRol = str_replace("_aprobada", "", $solicitud);
                        $estado = "aprobada";
                    } 
                    if (strpos($solicitud, "_denegada") !== false) 
                    {
                        $nuevoRol = str_replace("_denegada", "", $solicitud);
                        $estado = "denegada";
                    } 
                    if($estado !== "")
                    {
        ...
        $(document).ready(function () {
            <?php
                if (!empty($_SESSION["usuario"])) 
                {
                    ...
                    if ($tempUser->solicitud === "promotor_pendiente" || $tempUser->solicitud === "admin_pendiente" ) 
                    {
