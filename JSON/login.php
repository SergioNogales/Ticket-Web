...
if ($_SERVER["REQUEST_METHOD"] == "POST") 
{
    ...
    foreach ($usuarios as $usuario) 
    {
        if ($usuario["email"] === $email)
        {
            ...
            {
                $usuariodef = new usuario($usuario["email"], $usuario["password"], $usuario["rol"], $usuario["telefono"], $usuario["direccion"], $usuario["localidad"], $usuario["nombre"], $usuario["codigo_postal"], $usuario["tarjeta"], $usuario["mes_caducidad"], $usuario["year_caducidad"], $usuario["ccv"], $usuario["saldo"], $usuario["solicitud"]);
                $_SESSION["usuario"] = $usuariodef;
